    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#070102', color: '#ffffff', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px', fontFamily: 'system-ui, -apple-system, sans-serif', position: 'relative' }}>
      ...
      {!account ? (
        ...
        <div style={{ width: '100%', maxWidth: '360px', backgroundColor: '#0f0204', padding: '28px 22px', borderRadius: '28px', border: '1px solid rgba(153, 27, 27, 0.3)', boxShadow: '0 20px 40px rgba(0,0,0,0.9)' }}>
          ...
          {/* 🔥 Dashboard Renewal Status & Renewal Fee Box if Expired */}
          <div style={{ backgroundColor: '#040001', padding: '12px 14px', borderRadius: '14px', marginBottom: '14px', border: '1px solid #2a080c' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <span style={{ fontSize: '10px', color: '#9ca3af', fontWeight: '600', display: 'block' }}>Renewal Time Status:</span>
                <span style={{ color: isExpired ? '#ef4444' : '#4ade80', fontWeight: '700', fontSize: '12px', fontFamily: 'monospace' }}>
                  {isExpired ? "Expired" : (timeLeft !== null ? formatTime(timeLeft) : "Active")}
                </span>
              </div>
            </div>
